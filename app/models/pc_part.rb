# Manage the connection to database for one PhysicalColor association with
# other PhysicalColor
class PcPart < ApplicationRecord
  belongs_to :physical_color
  belongs_to :another_color,
             class_name: 'PhysicalColor', foreign_key: 'another_color_id'
  ...
  def another_color_is_already_stored
    if another_color && another_color.persisted?
      true
    else
      errors.add(:another_color,
                 'It is not saved yet; It must exist in database to use it')
    end
  end
end
